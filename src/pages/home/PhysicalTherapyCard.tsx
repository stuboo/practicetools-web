  ...
  return (
    <div className="border-2 border-gray-300 rounded-md bg-white p-4 transition-[border] hover:border-gray-600">
      ...
      <div className="flex text-lg items-center gap-2">
        ...
        <span className="bg-gray-500 text-white rounded-lg px-2 py-1 ml-3 text-sm flex justify-center items-center">
          {therapist.zip}
        </span>
        <span className="bg-blue-800 text-white rounded-lg px-2 py-1 ml-3 text-sm flex justify-center items-center">
          {Number(therapist.distance).toFixed(1)} miles
        </span>
      ...
      <div className="flex gap-6 mt-8">
        ...
        {therapist.referral_form_url && (
          <a href={therapist.referral_form_url} target="_blank">
            <Button
              iconLeft={<MdOutlineDownload size={24} />}
              title="Referral Form"
              colorScheme="blue"
              disabled={therapist.referral_form_url ? false : true}
            />
          </a>
        )}
        {!therapist.referral_form_url && (
